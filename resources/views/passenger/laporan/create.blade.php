...
@section('title', 'Buat Laporan Baru - Ngangkot')
...
<div class="bg-slate-50 min-h-screen py-12 px-6">
    <div class="max-w-3xl mx-auto">
        ...
        {{-- Header --}}
        <div class="mb-8">
            <h1 class="text-3xl font-extrabold text-slate-900 mb-2">Buat Laporan Baru</h1>
            <p class="text-slate-600">Bantu tingkatkan layanan angkot dengan melaporkan masalah atau memberikan masukan.</p>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-5 mb-8">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h4 class="font-bold text-blue-900 mb-1">Tips Membuat Laporan</h4>
                    <ul class="text-sm text-blue-700 space-y-1">
                        <li>• Jelaskan masalah dengan detail dan jelas</li>
                        <li>• Sertakan informasi trayek/angkot jika relevan</li>
                        <li>• Lampirkan bukti foto jika tersedia</li>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            <form action="{{ route('passenger.laporan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="isi" 
                              id="isi" 
                              rows="6"
                              placeholder="Jelaskan detail laporan Anda di sini. Sertakan informasi seperti waktu kejadian, lokasi, nomor angkot (jika ada), dan kronologi kejadian..."
                              class="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-slate-50 text-slate-700 font-medium focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition resize-none @error('isi') border-rose-300 bg-rose-50 @enderror">{{ old('isi') }}</textarea>
                    @error('isi')
                        <p class="text-rose-500 text-sm mt-2 flex items-center gap-1">
                            <i data-lucide="alert-circle" class="w-4 h-4"></i>
                            {{ $message }}
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <label for="bukti_foto" 
                               class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition @error('bukti_foto') border-rose-300 bg-rose-50 @enderror"
                               id="dropzone">
                            <div class="flex flex-col items-center justify-center py-6" id="upload-prompt">
                                <div class="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mb-3">
                                    <i data-lucide="image-plus" class="w-6 h-6 text-slate-400"></i>
                                </div>
                            ...
                            <div class="hidden" id="preview-container">
                                <img id="preview-image" src="" alt="Preview" class="max-h-32 rounded-xl">
                            </div>
                        </label>
                    </div>
                    @error('bukti_foto')
